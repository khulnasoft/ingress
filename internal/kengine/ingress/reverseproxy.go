...
func (p ReverseProxyPlugin) IngressHandler(input converter.IngressMiddlewareInput) (*kenginehttp.Route, error) {
	path := input.Path
	ing := input.Ingress
	...
	handler := reverseproxy.Handler{
		TransportRaw: kengineconfig.JSONModuleObject(transport, "protocol", "http", nil),
	...
	handlerModule := kengineconfig.JSONModuleObject(
		handler,
		"handler",
		"reverse_proxy",
		nil,
...
// Copied from https://github.com/khulnasoft/kengine/blob/21af88fefc9a8239a024f635f1c6fdd9defd7eb7/modules/kenginehttp/reverseproxy/reverseproxy.go#L270-L286
