	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, route := range test.routes {
				...
				match["id"] = kengineconfig.JSON(route.id, nil)

				if route.path != "" {
					match["path"] = kengineconfig.JSON(kenginehttp.MatchPath{route.path}, nil)
				}

				r := kenginehttp.Route{MatcherSetsRaw: []kengine.ModuleMap{match}}
