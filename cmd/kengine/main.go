...
import (
	...
	"github.com/khulnasoft/ingress/internal/kengine"
...
func main() {
	// parse any flags required to configure the kengine ingress controller
	cfg := parseFlags()
	...
	if cfg.WatchNamespace == "" {
		...
		logger.Warn("-namespace flag is unset, kengine ingress controller will monitor ingress resources in all namespaces.")
	}
	...
	c := controller.NewKengineController(logger, kubeClient, cfg, kengine.Converter{}, stopCh)

	// start the ingress controller
	logger.Info("Starting the kengine ingress controller")
	go c.Run()
